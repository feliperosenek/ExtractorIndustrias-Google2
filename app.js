...
async function validaGoogle() {
  try {
    ...
    console.log(" ");
    console.log(
      "  ▶▶▶ " + (getIndustrias.length - 1) + " industrias carregadas"
    ); // conta o número de industrias nao atualizadas

    for (let i = 0; i < getIndustrias.length; i++) {
      ...
      if (i == 0) {
        ...
        console.log(" ");
        console.log(
          " 🔎   Buscando informações sobre: " + getIndustria[0].fantasia
        );
      ...
      var linkItemResult = dom.window.document.querySelectorAll(".yuRUbf > div > span > a"); // busca a classe do título da pesquisa e extrai o href do link

      // --> VERIFICA SE ABRE O CARTÃO DA EMPRESANO GOOGLE MEU NEGÓCIO
      var verifyGoogle = dom.window.document.querySelector("#rhs");

      if (verifyGoogle) {
        // informa ao banco a verificação se está no Google
        verifyGoogle = 1;
      } else {
        verifyGoogle = 0;
      }
      ...
      for (let t = 0; t < linkItemResult.length; t++) {
        // resultado dos links da pagina de pesquisa
        ...
        if (
          ...
          link.includes("profile") === false &&
          link.includes("login") === false
        ) {
          //verifica se o link é Facebook
          ...
          await delay(7000);
          dataPage2 = await page2.evaluate(
          ...
          for (n = 0; n < getDataFaceook.length; n++) {
            ...
            if (contentRow.includes("@", ".com", ".br")) {
              // se tiver incluso é email
            ...
            if (
              ...
              // se tiver incluso é telefone
              var getFacebookFone = contentRow;
            }

            if (contentRow.includes("+", "55")) {
              //se tiver incluso é Whatsapp
              var getFacebookWhatsapp = contentRow;
            }

            if (contentRow.includes("http", "//")) {
              // se tiver incluso é site
              var getFacebookSite = contentRow;
            }
          }

          var getDataFacebookAbout = domPage2.window.document.querySelector(
            ".kvgmc6g5.cxmmr5t8.oygrvhab.hcukyx3x.c1et5uql"
          ); // pega os dados Sobre no Facebook

          if (getDataFacebookAbout) {
            if (getDataFacebookAbout.textContent.includes("Ver mais")) {
              //se tiver "Ver mais" no texto....

              await delay(3000);
              page2.click(
                ".kvgmc6g5.cxmmr5t8.oygrvhab.hcukyx3x.c1et5uql > div > div"
              ); // ...clica para abrir
              await delay(3000);

              dataPage3 = await page2.evaluate(
                () => document.querySelector("*").outerHTML
              ); // carrega o HTML de página
              domPage3 = new JSDOM(dataPage3);
              await delay(2000);

              var getDataFacebookAbout = domPage3.window.document.querySelector(
                ".kvgmc6g5.cxmmr5t8.oygrvhab.hcukyx3x.c1et5uql"
              ); // pega os dados com o texto expandido
              facebookAbout = facebookAbout.split("Ver menos"); // retira o texto
              facebookAbout = facebookAbout[0];
            } else {
              // se não tiver o termo "Ver mais" pega o texto completo
              var getDataFacebookAbout = domPage2.window.document.querySelector(
                ".kvgmc6g5.cxmmr5t8.oygrvhab.hcukyx3x.c1et5uql"
              );
        ...
        if (link.includes("linkedin") && link.includes("company") === true) {
          // se tiver incluso é Linkedin
          var linkedinLink = link;
        }

        // --> VERIFICA INSTAGRAM
        if (link.includes("instagram")) {
          // se tiver incluso é Instagram
        ...
        for (var r = 0; r < explodeNameIndustry.length; r++) {
          // verifica se o nome da empresa está também no linkSite
          ...
          if (
            link.includes(nameIndustriesVerify) === true &&
            nameIndustriesVerify != "www.e" &&
            nameIndustriesVerify != "www.de"
          ) {
            //filtro para obter o site
      ...
      var telefone1 = dom.window.document.querySelector(".LrzXr > a > span"); // pega o telefone no Google
      ...
      var getDataBase = await sequelize.query(
        "SELECT id_catalogo, nome, endereco, ddd_telefone, telefone, ddd_telefone2, telefone2, email, email_compras, email_vendas, pagina_web, descricao, ddd_celular, celular, facebook, instagram, linkedin FROM `catalogo` WHERE id_catalogo =  '" +
          results.id +
          "' ",
        {
          type: QueryTypes.SELECT,
      ...
      if (
        getDataBase[0] === undefined ||
        results.telefone == null ||
        results.telefone2 == null
      ) {
        // se o banco estiver vazio ou telefone for nulo, não faz nada
      } else {
        if (results.telefone.includes("(")) {
          // verifica se o resultado é um telefone
        ...
        if (telGoogle == telFacebook) {
          // se os telefones forem iguais, deixa só um
          telFacebook = "";
          dddTelefone2 = "";
        }
        ...
        if (!dddTelefone) {
          // se o telefone for vazio, pega o original
        ...
        if (getDataBase[0].email == getFacebookEmail) {
          //se os email foram iguai, deixa um só
          getFacebookEmail = "";
        }
        ...
        if (!results.site) {
          // se o site for vazio, pega o original
          results.site = getDataBase[0].pagina_web;
        }
        if (!endereco || endereco == null) {
          // se o enredeço for vazio, pega o original
        ...
        await sequelize.query(
          "UPDATE catalogo SET ddd_telefone='" +
            dddTelefone +
            "', telefone='" +
            telGoogle +
            "', ddd_telefone2='" +
